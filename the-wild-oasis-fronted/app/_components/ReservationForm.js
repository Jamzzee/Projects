...
function ReservationForm({ cabin, user, settings }) {
  ...
  const startDate = range?.from || null;
  const endDate = range?.to || null;
  const numNights = startDate && endDate && differenceInDays(endDate, startDate);
  const cabinPrice = numNights * (regularPrice - discount);
  ...
  return (
    <div className="mx-auto w-full max-w-2xl sm:max-w-full sm:px-4">
      {/* User info */}
      <div className="flex items-center justify-between rounded-t-md bg-primary-800 px-4 py-2 text-primary-300 sm:px-6">
        <p>Logged in as </p>
      ...
      <form
        ...
        className="flex flex-col gap-5 rounded-b-md bg-primary-900 px-4 py-6 text-base sm:px-6 sm:py-8 sm:text-lg"
      >
        {/* Number of guests */}
        <div className="space-y-2">
          <label htmlFor="numGuests">How many guests?</label>
          <select
            name="numGuests"
            id="numGuests"
            className="w-full rounded-sm bg-primary-200 px-3 py-2 text-primary-800 shadow-sm"
            required
            value={numGuests}
            onChange={handleNumberGuests}
          >
        ...
        <div className="space-y-2">
          ...
          <textarea
            name="observations"
            id="observations"
            className="w-full resize-none rounded-sm bg-primary-200 px-3 py-2 text-primary-800 shadow-sm"
            placeholder="Any pets, allergies, special requirements, etc.?"
          />
        </div>
        ...
        <div className="flex items-center justify-end gap-4">
          {!(startDate && endDate) ? (
            <p className="text-sm text-primary-300 sm:text-base">
              Start by selecting dates
            </p>
          ) : (
            <SubmitButton pendingLabel="Reserving">Reserve now</SubmitButton>
